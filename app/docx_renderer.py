...
class CoordinateDocxRenderer:
    ...
    def render(self, json_files: list[Path], out_path: Path) -> None:
        ...
        for page_no, json_path in enumerate(json_files):
            ...
            self._render_page(doc, page, data, sx, sy)
        self.pdf.close()
        doc.save(out_path)

    def _configure_section(self, section, width_pt, height_pt):
        section.page_width = Pt(width_pt); section.page_height = Pt(height_pt)
    ...
    def _render_page(self, doc, page, data, sx, sy):
        page_w, page_h = page.rect.width, page.rect.height
        # Preserve embedded PDF images, but never place a full-page scan over OCR text.
        seen = set()
        ...
        for item in data.get("table_res_list") or []:
            if isinstance(item, dict) and isinstance(item.get("pred_html"), str):
                table_htmls.append(item["pred_html"])
        table_index = 0
        for block in blocks:
            ...
            if label in {"image", "figure", "chart"}: continue
    ...
    def _add_text_box(self, doc, text, bbox):
        x0,y0,x1,y1 = bbox; width=max(1,x1-x0); height=max(1,y1-y0)
        font_pt=max(6,min(36,height*0.72))
        p=doc.add_paragraph(); self._zero_paragraph(p)
        pict=OxmlElement("w:pict"); shape=self._shape(x0,y0,width,height)
        txbx=OxmlElement("v:textbox"); content=OxmlElement("w:txbxContent")
        wp=OxmlElement("w:p"); wr=OxmlElement("w:r"); rp=OxmlElement("w:rPr")
        sz=OxmlElement("w:sz"); sz.set(qn("w:val"),str(max(2,int(font_pt*2)))); rp.append(sz); wr.append(rp)
        wt=OxmlElement("w:t"); wt.text=text; wr.append(wt); wp.append(wr); content.append(wp)
        txbx.append(content); shape.append(txbx); pict.append(shape); p._p.append(pict)

    def _add_table_box(self, doc, table_html, bbox):
        root=html.fromstring(table_html); table=root if root.tag.lower()=="table" else root.find(".//table")
        if table is None: return self._add_text_box(doc, root.text_content().strip(), bbox)
        rows=[]
        for row in table.xpath(".//tr"):
            rows.append([(c.text_content().strip(), c.tag.lower()=="th") for c in row.xpath("./th|./td")])
        if not rows: return
        x0,y0,x1,y1=bbox; p=doc.add_paragraph(); self._zero_paragraph(p)
        pict=OxmlElement("w:pict"); shape=self._shape(x0,y0,x1-x0,y1-y0); txbx=OxmlElement("v:textbox"); content=OxmlElement("w:txbxContent")
        tbl=OxmlElement("w:tbl"); self._build_table(tbl,rows); content.append(tbl); txbx.append(content); shape.append(txbx); pict.append(shape); p._p.append(pict)

    def _build_table(self,tbl,rows):
        tbl_pr=OxmlElement("w:tblPr"); borders=OxmlElement("w:tblBorders")
        for edge in ("top","left","bottom","right","insideH","insideV"):
            el=OxmlElement(f"w:{edge}"); el.set(qn("w:val"),"single"); el.set(qn("w:sz"),"4"); el.set(qn("w:space"),"0"); el.set(qn("w:color"),"808080"); borders.append(el)
        ...
        for cells in rows:
            ...
            for text,header in cells:
                ...
                if header:
                    rp=OxmlElement("w:rPr"); rp.append(OxmlElement("w:b")); r.append(rp)
    ...
    def _add_floating_image(self,doc,blob,rect):
        ...
        p=doc.add_paragraph(); self._zero_paragraph(p); run=p.add_run()
        inline_shape=run.add_picture(io.BytesIO(blob),width=Pt(max(1,x1-x0)),height=Pt(max(1,y1-y0)))
        inline=inline_shape._inline; anchor=OxmlElement("wp:anchor")
        ...
        for tag,value in (("positionH",x0),("positionV",y0)):
            pos=OxmlElement(f"wp:{tag}"); pos.set("relativeFrom","page"); off=OxmlElement("wp:posOffset"); off.text=str(int(value*EMU_PER_INCH/PT_PER_INCH)); pos.append(off); anchor.append(pos)
        ...
        eff=OxmlElement("wp:effectExtent"); [eff.set(k,"0") for k in ("l","t","r","b")]; anchor.append(eff); anchor.append(OxmlElement("wp:wrapNone"))
        for child in list(inline): anchor.append(child)
        inline.getparent().replace(inline,anchor)

    def _shape(self,x,y,width,height):
        shape=OxmlElement("v:shape"); shape.set(qn("id"),"_x0000_s"+str(abs(hash((x,y,width,height)))%900000+100000)); shape.set(qn("type"),"#_x0000_t202")
        shape.set(qn("style"),f"position:absolute;margin-left:{x}pt;margin-top:{y}pt;width:{width}pt;height:{height}pt;mso-wrap-style:none"); shape.set(qn("stroked"),"f"); shape.set(qn("filled"),"f"); return shape
    ...
    def _page_index(self,path,fallback):
        m=re.search(r"page-(\d+)",path.name); return int(m.group(1)) if m else fallback
