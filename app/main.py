...
@app.get("/health")
def health():
    return {"status":"ok","gemini":bool(os.getenv("GEMINI_API_KEY")),"model":os.getenv("GEMINI_MODEL","gemini-3.7-flash"),"fallback_model":os.getenv("GEMINI_FALLBACK_MODEL","gemini-3.6-flash")}
...
async def tool(tool: str, files: list[UploadFile] = File(...), pages: str = "", angle: int = 90):
    ...
    task=uuid4().hex; work=BASE/"jobs"/task; work.mkdir(parents=True); saved=[]
    try:
        for i,f in enumerate(files):
            if not f.filename: continue
            ext=Path(f.filename).suffix.lower()
            if tool != "images-to-pdf" and ext != ".pdf": raise HTTPException(400,"PDF required")
        ...
        if tool=="merge": out=OUTPUT/f"{task}.pdf"; merge_pdfs(saved,out)
        elif tool=="split": targets=split_pdf(saved[0],work); shutil.make_archive(str(OUTPUT/task),"zip",work); return {"download_url":f"/api/v1/files/{task}.zip","count":len(targets)}
        elif tool=="extract":
            nums=[]
            for part in pages.split(","):
                if "-" in part: a,b=map(int,part.split("-")); nums.extend(range(a,b+1))
        ...
        elif tool=="compress": out=OUTPUT/f"{task}.pdf"; compress_pdf(saved[0],out)
            ...
            except json.JSONDecodeError as exc: raise HTTPException(502,f"Gemini returned invalid JSON: {exc.msg}")
        ...
        if tool in {"pdf-to-word","pdf-to-excel"}: raise HTTPException(502,f"Document conversion failed: {exc}") from exc
